...
import os.path
import matplotlib.patches as mpatches # for debugging
...
class Trace:
    ...
    def addtomap(self, ax, color='black'):
        update_axes_extent(ax, self.latlon)
    ...
    def check_points(self):
        ...
        distances = np.sqrt((eastings[1:] - eastings[:-1]) **2 + (northings[1:] - northings[:-1]) **2)
...
class Task:
    '''
    Class to hold task's turnpoint trigraphs and lat/lon info.  Will also contain
    FAI sector or barrel shapes for each turnpoint when I get around to coding them.
    '''
    def __init__(self, turnpoint_list=[], latlon_list=None, barrels=False):
        ...
        if len(self.names):
            if latlon_list:
                if len(self.names) != len(latlon_list):
                    raise UserWarning('Turnpoint name and location lists inconsistent lengths')
                else:
        ...
        self.geoms = []  # list of barrel or FAI sector shapely polygons.  NOT multipolygon as need order.
        
        if barrels:
            for name in self.names:
                lon, lat = self.latlon[name]
                self.geoms.append(sgeom.Point(GEOM_CRS.transform_point(lon, lat, SOURCE_CRS)).buffer(500))
        else:
            pass
    ...
    def addtomap(self, ax, color=None):
        if len(self.names):
            if len(self.geoms):
                ax.add_geometries(self.geoms, GEOM_CRS, facecolor='w', alpha=0.5)
                for lon, lat in self.latlon.values():
                    ax.add_patch(mpatches.Circle(GEOM_CRS.transform_point(lon, lat, SOURCE_CRS), 50, edgecolor='MidnightBlue', facecolor='none', transform=GEOM_CRS))
            task_lon, task_lat = zip(*[self.latlon.get(tp) for tp in self.names])
            update_axes_extent(ax, sgeom.LineString([self.latlon.get(tp) for tp in self.names]))
            ax.plot(task_lon, task_lat, transform=SOURCE_CRS, color=color)
            for name, coord in self.latlon.iteritems():
                ax.annotate(name, xy=ax.projection.transform_point(coord[0], coord[1], SOURCE_CRS),
                           textcoords='offset points', xytext=(5,5), bbox=dict(boxstyle="round", fc="w", alpha=0.7))
...
def read_headers(ifp):
    '''Read header fields from an open IGC logger file, and store information in a dictionary'''
    headers = {}
    while True:
        line = ifp.readline()
...
def write_header_records(logger_filelist, record_file, append=False):
    '''Read the headers from a list of IGC files and write them into a text file in csv format.'''
    ...
    for logfile in logger_filelist:
        ...
        if headers.get('pilot'):
            headers['pilot'] = headers['pilot'].replace('.', ' ').replace('_', ' ')
...
class GliderFlight:
    '''
    Class to hold information about a glider flight: a trace, a (possibly empty) task, and 
    pilot/glider/date info in the header.  Initialised with an IGC logger file.
    '''
    def __init__(self, filename, barrels=False, skip_declaration=False):
	    ...
	    for line in ifp:
                if line.startswith('B'):
                    self.trace.times.append(datetime.time(int(line[1:3]), int(line[3:5]), int(line[5:7])))
...
def update_axes_extent(ax, geom):
    '''
    Updates axes extent to comfortably fit the geometry.
    '''
    pass
    #buffer_size = math.sqrt(geom.envelope.area)/20.
    #newgeom = geom.envelope.buffer(buffer_size)
    
    #old_extent = ax.get_extent(SOURCE_CRS)
    #if old_extent[1]-old_extent[0] < 100:

        #oldgeom = sgeom.LineString([old_extent[0:3:2], old_extent[1:4:2]])
        #newgeom = newgeom.union(oldgeom)
        
    #(minlon, minlat, maxlon, maxlat) = newgeom.bounds    
...
def plotmap(flights, zoom=10, tracecolors=['black'], taskcolors=['Crimson'], aspectratio=None, terrain=False):
    '''
    Plot one or more GliderFlight instances on a map.
    
    Arguments:
    * flights: A (list of) GliderFlight instance(s)
    * zoom: detail level of map image.  10 is recommended for large tasks, 12 for local
        (within gliding range of airfield) tasks
    ...
    * terrain: boolean.  False to plot OpenStreetMap map, True to plot Google terrain
    '''
    ...
    ax.annotate('$\copyright$ Mapbox, $\copyright$ OpenStreetMap', xy=(2, 2), xycoords='axes points',
                 fontsize='small')
...
def plotaltitude(flights, ax, colors=['black']):
    '''
    Plot a graph of altitude against time from one or more GliderFlight instances.
    '''
    if type(flights) is not list:
        flights = [flights]
    ...
    for flight, color in zip(flights, colors):
        x = [ datetime.datetime.combine(flight.headers.get('date'), t) for t in flight.trace.times]
...
def plotflight(filename, pilotname=None, gliderid=None, tp_list=None, barrels=False, altitude=True, **kwargs):
    '''
    ...
    * barrels: bolean indicating True for half km barrel turnpoints, False for FAI sectors
...
if __name__ == '__main__':
    #plotflight('2013-07-05-XCS-AAA-01.igc', 'Rowan Smith', 'CLM')
    #plotflight('2013-07-05-XCS-AAA-01.igc', 'Rowan Smith', 'CLM', ['NHL','WAN','AVE','PRK','NHL'])
    plotflight('ruth_silver_height_flight.IGC', 'Ruth Comer', 'KHA', ['NHL','CUL','HHL','CLS','NHL'], barrels=True, zoom=12)
    #plotflight('2015-01-02-XCM-49M-02.igc', zoom=11, terrain=True, aspectratio=math.sqrt(2.))
    #plotflight('ruth_silver_height_flight.IGC', 'Ruth Comer', 'KHA', ['NHL','CUL','HHL','CLS','NHL'], 12, False)
    #ax = plotmap(GliderFlight('ruth_silver_height_flight.IGC'), plt.subplot, 12, aspectratio=3)
    #plt.show()
    #plotflight('38JGC171.IGC', tp_list=['NHL','TIE','SAL','WDG','NHL'])
    #plotflight('41NX1Y62.igc')
    #plotflight('rick_silver_height.IGC', 'Rick Andrews', 'KHA', zoom=12)
    #plotflight('43AG4YW1.IGC', 'Paul Tickner', 'WE4')
    #plotflight('pete.igc', 'Captain Slow', 'JZK', ['NHL','CUL', 'NHL'], zoom=12)
    #plotflight('sant.IGC')
    #plotflight('43FV7D71.igc', tp_list=['CLS','WEG','TIE','CUL','HHL','NHL'], zoom=12)
    #plotflight('pete_ccx.igc', tp_list = ['NHL', 'TIV', 'NHL', 'TIV', 'CCX', 'NHL'], zoom=12)
