...
def plotonmap(lgdir, title, imagefile, turnpointlist=None, plot_extras=None,
              labels_use=None, zoom=9):
    ...
    # Plot traces
    ax1 = plotmap(flights[::-1], zoom=zoom,
                  tracecolors=fixed_colors + colors[::-1],
    ...
    lines = [mlines.Line2D([], [], color=color, label=label) for
             (color, label) in zip(colors, labels_use)]
    ...
    for distance in range(10, 181, 10):
        ...
        ax.annotate('%skm' % distance,
                    xy=ax.projection.transform_point(
                        DSGC_OS[0]+distance*820 + 1500,
                        DSGC_OS[1]+distance*570, OS_CRS),
                    color=color,
                    fontsize='small',
                    ha='left', va='top')
